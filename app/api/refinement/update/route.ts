...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => null) as {
    ...
    problemConfirmations?: number | null;
    positiveSignals?: number | null;
    commitmentSignals?: number | null;
    negativeSignals?: number | null;
  } | null;
  ...
  const { data: test } = await supabase
    ...
    .select("id,project_id,test_type,status")
    .eq("id", body.testId)
  ...
  if ([sample, confirmations, positives, commitments, negatives].some((value) => !Number.isFinite(value) || value < 0)) {
    return NextResponse.json({ error: "Evidence counts must be non-negative numbers." }, { status: 400 });
  }
  if ([confirmations, positives, commitments, negatives].some((value) => value > sample)) {
    return NextResponse.json({ error: "Evidence counts cannot exceed the number of people tested." }, { status: 400 });
  }
  ...
  if (body.status === "passed") {
    ...
    if (!signal.thresholdMet) {
      ...
      return NextResponse.json({
        error: `${signal.summary} The test has been recorded as needing another iteration rather than unlocking Build.`,
        test: { status: nextStatus, validation_score: validationScore, signal_summary: signalSummary },
      }, { status: 422 });
    }
  }
  ...
  const { data: updated, error } = await supabase
    ...
    .eq("id", body.testId)
    .select("id,status,test_type,validation_score,signal_summary")
    .single();
