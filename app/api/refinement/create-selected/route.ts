...
export async function POST(request: Request) {
  ...
  if (report.decision !== "refine") return NextResponse.json({ error: "This opportunity does not currently require refinement." }, { status: 400 });

  const { data: existing } = await supabase.from("validation_tests")
    .select("id,status")
    .eq("project_id", body.projectId)
    .eq("opportunity_id", opportunity.id)
    ...
    .limit(1)
    .maybeSingle();
  if (existing) return NextResponse.json({ test: existing, reused: true });
  ...
  return NextResponse.json({ test, reused: false });
}
