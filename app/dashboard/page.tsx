    ...
    <main className="pf-shell min-h-screen text-[#171714]">
      ...
      <div className="mx-auto max-w-[1440px] px-5 py-8 sm:px-8 lg:px-10 lg:py-12">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {[["Projects", projects?.length ?? 0, "Active workspaces"], ["Research mode", "Evidence", "Sources stay attached to insights"], ["Next step", projects?.length ? "Research" : "Discover", projects?.length ? "Continue a project" : "Create your first project"]].map(([label, value, description]) => (
            <div key={label as string} className="pf-card pf-card-hover rounded-[26px] p-6">
              <p className="pf-mono text-[9px] font-semibold uppercase tracking-[.16em] text-[#999991]">{label}</p>
              <p className="pf-display mt-5 text-3xl font-semibold tracking-tight">{value}</p>
              <p className="mt-1 text-xs text-[#707069]">{description}</p>
            </div>
          ))}
        </div>
        ...
        <section className="mt-12">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {projects?.length ? projects.map((project) => {
              const currentStage = Math.min(Math.max(Number(project.current_stage ?? 1), 1), 5);
              return <Link key={project.id} href={`/projects/${project.id}`} className="pf-card-hover group rounded-[28px] border border-[#dcdcd4] bg-white p-6">
                ...
                <div className="mt-7 flex items-center gap-1.5">{stageLabels.map((label, index) => <span key={label} title={label} className={`h-1.5 flex-1 rounded-full ${index < currentStage ? "bg-[#171714]" : "bg-[#e7e7e0]"}`} />)}</div>
                <div className="mt-3 flex items-center justify-between text-[10px] font-semibold text-[#8a8a82]"><span>Stage {String(currentStage).padStart(2, "0")} / 05</span><span>{stageLabels[currentStage - 1]}</span></div>
              </Link>;
