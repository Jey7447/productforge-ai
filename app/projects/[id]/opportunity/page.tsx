...
export default async function OpportunityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const score = Math.round(Number(selected?.opportunity_score ?? 0));
  const confidence = Math.round(Number(selected?.confidence_score ?? 0));
  const metrics = [
    ["Demand", selected?.demand_score],
    ["Problem intensity", selected?.problem_intensity_score],
    ["Competition gap", selected?.competition_gap_score],
    ["Monetization", selected?.monetization_score],
    ["Specificity", selected?.specificity_score],
    ["Buildability", selected?.buildability_score],
  ] as const;
            ...
            {selected && <>
              <div className="mt-12 grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
                <section className="pf-card pf-card-hover rounded-[30px] p-7">
                  <div className="flex items-center justify-between"><p className="pf-mono text-[9px] font-bold uppercase tracking-[.18em] text-[#8a8a82]">Selected hypothesis</p><span className="pf-mono rounded-full bg-[#eef5cf] px-2.5 py-1 text-[9px] font-bold uppercase tracking-[.12em] text-[#596b00]">{selected.status === "selected" ? "Selected" : "Current leader"}</span></div>
                  <h2 className="pf-display mt-3 text-3xl font-semibold">{selected.title}</h2>
                  <p className="mt-5 text-sm leading-7 text-[#5f5f58]">{selected.proposed_product ?? "The research engine did not record a product concept."}</p>
                  <div className="mt-7 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-[#f4f4f0] p-4"><p className="pf-mono text-[9px] uppercase tracking-[.12em] text-[#999991]">Audience</p><p className="mt-2 text-sm font-semibold">{selected.target_audience ?? "Defined from research"}</p></div><div className="rounded-2xl bg-[#f4f4f0] p-4"><p className="pf-mono text-[9px] uppercase tracking-[.12em] text-[#999991]">Product type</p><p className="mt-2 text-sm font-semibold">{selected.product_type ?? "Digital product"}</p></div></div>
                </section>
                <section className="pf-card pf-glow rounded-[30px] bg-[#ecece6] p-7">
                  ...
                  <div className="mt-5 space-y-4">{metrics.map(([label, value]) => { const numeric = Math.round(Number(value ?? 0)); return <div key={label}><div className="flex items-end justify-between"><span className="text-xs text-[#73736d]">{label}</span><span className="pf-mono text-[10px] font-semibold">{String(numeric).padStart(2, "0")}</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white"><div className="h-full rounded-full bg-[#171714]" style={{ width: `${numeric}%` }} /></div></div>; })}</div>
                </section>
              </div>
