    ...
    <div className="mt-8 space-y-5">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-[30px] bg-[#dff77a] p-7 text-[#171714]">
          ...
          ) : <><p className="mt-4 text-sm leading-7 text-[#41431f]">{worksheet?.content?.purpose || "Use this worksheet to turn the module into concrete work."}</p><div className="mt-6 space-y-2">{(worksheet?.content?.prompts ?? []).map((prompt, index) => <div key={`${worksheet.id}-${index}`} className="rounded-xl bg-white/60 px-4 py-3 text-xs font-medium leading-5 text-[#41431f]"><span className="mr-2 font-bold">{index + 1}.</span>{prompt}</div>)}</div></>}
        </div>
      </section>
