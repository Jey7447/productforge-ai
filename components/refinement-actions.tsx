    ...
    try {
      ...
      setMessage(
        nextStatus === "passed"
          ...
          : nextStatus === "failed"
            ? "Test failed. Review the evidence and run the next iteration."
    ...
    <div className="mt-6 rounded-2xl bg-[#f5f5f2] p-5">
      <div className="grid gap-4 md:grid-cols-2">
        <label className="block">
          <span className="pf-mono text-[10px] font-bold uppercase tracking-[.16em] text-[#8a8a82]">What did you learn?</span>
          <textarea
            value={notes}
        ...
        <label className="block">
          <span className="pf-mono text-[10px] font-bold uppercase tracking-[.16em] text-[#8a8a82]">Outcome</span>
          <textarea
            value={outcome}
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button disabled={inputsLocked} onClick={() => updateTest("in_progress")} className="rounded-full border border-[#d8d8d0] bg-white px-4 py-2 text-xs font-semibold text-[#171714] disabled:cursor-not-allowed disabled:opacity-50">Mark in progress</button>
        <button disabled={!canPass} onClick={() => updateTest("passed")} className="rounded-full bg-[#171714] px-4 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">Pass test → Unlock build</button>
        <button disabled={!canFail} onClick={() => updateTest("failed")} className="rounded-full border border-[#e3b8b3] bg-white px-4 py-2 text-xs font-semibold text-red-700 disabled:cursor-not-allowed disabled:opacity-50">Fail / refine again</button>
      </div>

      <p className="mt-3 text-xs leading-5 text-[#73736d]">Build unlocks only after you record evidence and an outcome. ProductForge does not treat an unchecked test as proof.</p>
