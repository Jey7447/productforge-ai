...
export function buildResearchPlan(input: ResearchInput): ResearchQueryPlan[] {
  ...
  const context = [startingPoint, expertise && `expertise: ${expertise}`, problems && `problems: ${problems}`, productTypes && `product formats: ${productTypes}`].filter(Boolean).join("; ");
  ...
  ].map((plan) => ({ ...plan, query: `${plan.query} ${context}`.trim() }));
}
